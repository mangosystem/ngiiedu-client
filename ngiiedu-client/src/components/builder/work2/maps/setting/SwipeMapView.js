...
class SwipeMapView extends React.Component {
  ...
  removeLayer(ctrl) {
    ...
    for (let i=0; i<ctrl.layers.length; i++) {
      ctrl.removeLayer(ctrl.layers[i]);
    }
  ...
  changeLayer(layerId1, layerId2, ctrl, right) {
    ...
    this.setState({ layerId1, layerId2 });
    ...
    ajaxJson(
      ['GET',apiSvr+'/coursesWork/layers/'+layer+'.json'],
      null,
      function(res){
  ...
  componentWillReceiveProps(nextProps){
    ...
    let { ctrl, layerId1, layerId2 } = this.state;

    if (nextProps.maps.typeKind == 'HORIZONTAL') {
      ctrl.set('orientation','horizontal');
    } else if (nextProps.maps.typeKind == 'VERTICAL') {
      ctrl.set('orientation','vertical');
    }

    if (layerId1 != nextProps.items[0].pinoLayer && layerId2 != nextProps.items[1].pinoLayer) {
      let newLayerId1 = nextProps.items[0].pinoLayer;
      let newLayerId2 = nextProps.items[1].pinoLayer;
      this.removeLayer(ctrl);
      let newCtrl = this.removeLayer(ctrl);
      this.changeLayer(newLayerId1, newLayerId2, newCtrl);
    } else if (layerId1 != nextProps.items[0].pinoLayer) {
      let newLayerId1 = nextProps.items[0].pinoLayer;
      this.removeLayer(ctrl);
      let newCtrl = this.removeLayer(ctrl);
      this.changeLayer(newLayerId1, layerId2, newCtrl);
    } else if (layerId2 != nextProps.items[1].pinoLayer) {
      let newLayerId2 = nextProps.items[1].pinoLayer;
      this.removeLayer(ctrl);
      let newCtrl = this.removeLayer(ctrl);
      this.changeLayer(layerId1, newLayerId2, newCtrl, true);
    }
